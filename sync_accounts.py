...
for mapping in api_settings.mapping:
    accounts.append(get_transactions_period(http_session, api_settings.CUSTOMERID, mapping["ID"], startDate, endDate))
    ...
    if len(account_map["account"]) > 2:  # Only fetch YNAB transactions from accounts that are synced in YNAB
        try:
            ...
            api_response = api_instance.get_transactions_by_account(
                api_settings.budget_id, account_map["account"], since_date=startDate
            )
        except ApiException as e:
    ...
    for transaction_item in transactions:
        ...
        if transaction_item["transactionTypeCode"] == 200:  # Transfer between own accounts
            ...
            if payee:
                ...
                ynab_transaction.payee_name = (
                    (ynab_transaction.payee_name[:45] + "...")
                    if len(ynab_transaction.payee_name) > 49
                    else ynab_transaction.payee_name
                )
        else:
            ynab_transaction.payee_name = (
                (ynab_transaction.payee_name[:45] + "...")
